...
def create_database_and_tables():
    ...
    try:
        ...
        # Create database if it doesn't exist (local and hosted)
        ...
        # Track migrations so pull-from-GitHub on host runs only new changes
        db_migrations.ensure_migrations_table(cursor)
        ...
        # Create pig_feeding_records table to track per-pig feeding (amount, time) and link to stock
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS pig_feeding_records (
                id INT AUTO_INCREMENT PRIMARY KEY,
                pig_id INT NOT NULL,
                feed_id INT NOT NULL,
                farm_id INT NULL,
                amount_kg DECIMAL(10,2) NOT NULL,
                feeding_datetime DATETIME NOT NULL,
                notes TEXT,
                feed_stock_id INT NULL,
                created_by INT NOT NULL,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                ...
                INDEX idx_feeding_datetime (feeding_datetime)
            )
        """)
        ...
        """)
        print("Feeds table checked/created successfully")

        # Add animal_type to feeds so each feed is linked to pig, cow, or chicken
        try:
            cursor.execute("""
                ALTER TABLE feeds
                ADD COLUMN animal_type VARCHAR(20) NOT NULL DEFAULT 'pig'
                COMMENT 'Animal this feed is for: pig, cow, or chicken'
            """)
            print("Feeds table: animal_type column added")
        except Exception as e:
        ...
        try:
            ...
            pass
        print("Feed stock table checked/created successfully")
        
        # Create feed_settings table for age-based feed configurations
        cursor.execute("""
        ...
        """)
        print("Feed settings table checked/created successfully")
        ...
        # Create cow_feeding_records table to track per-cow feeding (amount, time) and link to stock
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS cow_feeding_records (
                id INT AUTO_INCREMENT PRIMARY KEY,
                cow_id INT NOT NULL,
                feed_id INT NOT NULL,
                farm_id INT NULL,
        ...
        """)
        print("Cow feeding records table checked/created successfully")

        # Create chicken_feeding_records table (requires chickens table to exist)
        try:
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS chicken_feeding_records (
                    id INT AUTO_INCREMENT PRIMARY KEY,
            ...
            """)
            print("Chicken feeding records table checked/created successfully")
        except Exception as e:
            if "chickens" in str(e).lower() or "foreign" in str(e).lower():
                print("Chicken feeding records table skipped (chickens table may not exist yet)")
            else:
                raise
        ...
        # ----- Cow vaccination & weight tables (same logic as pigs) -----
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS cow_vaccination_schedule (
                id INT AUTO_INCREMENT PRIMARY KEY,
                day_number INT NOT NULL,
                day_description VARCHAR(100),
                reason TEXT NOT NULL,
                ...
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                FOREIGN KEY (created_by) REFERENCES employees(id),
                UNIQUE KEY unique_cow_day (day_number)
            )
        """)
        print("Cow vaccination schedule table checked/created successfully")
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS cow_vaccination_records (
                id INT AUTO_INCREMENT PRIMARY KEY,
                cow_id INT NOT NULL,
                schedule_id INT NOT NULL,
                completed_date DATE NOT NULL,
                ...
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                FOREIGN KEY (cow_id) REFERENCES cows(id) ON DELETE CASCADE,
                FOREIGN KEY (schedule_id) REFERENCES cow_vaccination_schedule(id),
                FOREIGN KEY (completed_by) REFERENCES employees(id),
                UNIQUE KEY unique_cow_schedule (cow_id, schedule_id)
            )
        """)
        print("Cow vaccination records table checked/created successfully")
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS cow_weight_categories (
                id INT AUTO_INCREMENT PRIMARY KEY,
                start_age INT NOT NULL,
                end_age INT NOT NULL,
                category_name VARCHAR(100) NOT NULL,
                ...
                daily_gain DECIMAL(5,2) NOT NULL,
                created_by INT NOT NULL,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                FOREIGN KEY (created_by) REFERENCES employees(id)
        ...
        """)
        print("Cow weight categories table checked/created successfully")
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS cow_weight_settings (
                id INT AUTO_INCREMENT PRIMARY KEY,
                setting_name VARCHAR(100) NOT NULL,
                setting_value TEXT,
                setting_type ENUM('text', 'number', 'boolean', 'json') DEFAULT 'text',
                ...
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                FOREIGN KEY (created_by) REFERENCES employees(id),
                UNIQUE KEY unique_cow_setting (setting_name)
            )
        """)
        print("Cow weight settings table checked/created successfully")
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS cow_weight_records (
                id INT AUTO_INCREMENT PRIMARY KEY,
                cow_id INT NOT NULL,
                weight DECIMAL(8,2) NOT NULL,
                expected_weight DECIMAL(8,2) NULL,
                ...
                FOREIGN KEY (cow_id) REFERENCES cows(id) ON DELETE CASCADE,
                FOREIGN KEY (created_by) REFERENCES employees(id),
                INDEX idx_cow_weighing (cow_id, weighing_date)
            )
        """)
        print("Cow weight records table checked/created successfully")
        # Insert default cow weight categories if table is empty
        cursor.execute("SELECT COUNT(*) as count FROM cow_weight_categories")
        if cursor.fetchone()['count'] == 0:
            try:
                cursor.execute("SELECT id FROM employees LIMIT 1")
                emp = cursor.fetchone()
                created_by = emp['id'] if emp else 1
                cursor.execute("""
                    INSERT INTO cow_weight_categories (start_age, end_age, category_name, min_weight, max_weight, daily_gain, created_by) VALUES
                    (1, 90, 'Calf', 30, 80, 0.6, %s),
                    (91, 365, 'Heifer/Growing', 80, 250, 0.5, %s),
                    (366, 730, 'Young cow', 250, 450, 0.3, %s),
                    (731, 3650, 'Mature cow', 450, 650, 0.1, %s)
                """, (created_by, created_by, created_by, created_by))
                print("Default cow weight categories inserted successfully")
            except Exception as e:
                print(f"Error inserting cow weight categories: {e}")
        # Insert default cow vaccination schedule if empty
        cursor.execute("SELECT COUNT(*) as count FROM cow_vaccination_schedule")
        if cursor.fetchone()['count'] == 0:
            try:
                cursor.execute("SELECT id FROM employees LIMIT 1")
                emp = cursor.fetchone()
                created_by = emp['id'] if emp else 1
                cursor.execute("""
                    INSERT INTO cow_vaccination_schedule (day_number, day_description, reason, medicine_activity, dosage_amount, interval_duration, additional_notes, created_by) VALUES
                    (1, 'At birth', 'Colostrum and initial protection', 'Vitamin/mineral, colostrum supplement', 'As per vet', 'Once', 'Ensure colostrum within first 6 hours', %s),
                    (30, '1 month', 'Clostridial and respiratory', 'Clostridial vaccine, optional respiratory', '2ml', 'Single', 'As per herd health plan', %s),
                    (60, '2 months', 'Booster', 'Clostridial booster', '2ml', 'Booster', 'Repeat as per schedule', %s),
                    (180, '6 months', 'Pre-breeding', 'Brucellosis, BVD, IBR as per region', 'As per vet', 'As per protocol', 'Region-specific vaccines', %s),
                    (365, 'Annual', 'Adult herd', 'Annual boosters (clostridial, BVD, IBR, lepto)', 'As per vet', 'Yearly', 'Maintain herd immunity', %s)
                """, (created_by, created_by, created_by, created_by, created_by))
                print("Default cow vaccination schedule inserted successfully")
            except Exception as e:
                print(f"Error inserting cow vaccination schedule: {e}")
        # Insert default cow weight settings if empty
        cursor.execute("SELECT COUNT(*) as count FROM cow_weight_settings")
        if cursor.fetchone()['count'] == 0:
            try:
                cursor.execute("SELECT id FROM employees LIMIT 1")
                emp = cursor.fetchone()
                created_by = emp['id'] if emp else 1
                cursor.execute("""
                    INSERT INTO cow_weight_settings (setting_name, setting_value, setting_type, description, created_by) VALUES
                    ('weight_unit', 'kg', 'text', 'Default weight unit for cows', %s),
                    ('weighing_frequency', 'monthly', 'text', 'How often cows should be weighed', %s)
                """, (created_by, created_by))
                print("Default cow weight settings inserted successfully")
            except Exception as e:
                print(f"Error inserting cow weight settings: {e}")
        ...
        # Run migrations (adds missing columns / new schema changes; tracked so host gets updates on pull).
...
def inject_role_url():
    ...
    def role_url(path):
        """Generate URL based on user role - converts /admin/ to /manager/ for managers"""
        if 'employee_role' in session:
            role = session.get('employee_role', 'administrator')
...
def manager_farm_chicken_management():
    ...
    try:
        ...
        # Get total counts by chicken type
        cursor.execute("""
            SELECT 
                chicken_type,
...
def manager_pigs_analytics():
    ...
    return render_template('admin_pigs_analytics.html', user=user_data)

@app.route('/manager/cows-analytics')
def manager_cows_analytics():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    ...
    return render_template('admin_cows_analytics.html', user=user_data)
...
def manager_farm_breeding_records():
    ...
    return render_template('admin_farm_breeding_records.html', user=user_data)

@app.route('/manager/farm/completed-farrowings')
def manager_farm_completed_farrowings():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
...
@app.route('/manager/farm/vaccination-tracking')
def manager_farm_vaccination_tracking():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
...
@app.route('/manager/farm/insert-weight')
def manager_farm_insert_weight():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    ...
    return render_template('admin_farm_insert_weight.html', user=user_data)

# Comprehensive Manager Routes - All remaining admin routes
@app.route('/manager/farm/register')
def manager_farm_register():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
...
@app.route('/manager/farm/view/<int:farm_id>')
def manager_farm_view(farm_id):
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/health-management')
def manager_farm_health_management():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/cow-health')
def manager_farm_cow_health():
    """Cow Health page - vaccine & weight links only on this page"""
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
        'role': session['employee_role'], 'status': session.get('employee_status', 'active'),
        'email': f"{session['employee_name'].lower().replace(' ', '.')}@farm.com"
    }
    return render_template('admin_farm_cow_health.html', user=user_data)

@app.route('/manager/farm/cow-vaccination-tracking')
def manager_farm_cow_vaccination_tracking():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
    ...
    return render_template('admin_farm_cow_vaccination_tracking.html', user=user_data)

@app.route('/manager/farm/cow-vaccination-settings')
def manager_farm_cow_vaccination_settings():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
    ...
    return render_template('admin_farm_cow_vaccination_settings.html', user=user_data)

@app.route('/manager/farm/cow-weight-assessment')
def manager_farm_cow_weight_assessment():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
    ...
    return render_template('admin_farm_cow_weight_assessment.html', user=user_data)

@app.route('/manager/farm/cow-weight-settings')
def manager_farm_cow_weight_settings():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
    ...
    return render_template('admin_farm_cow_weight_settings.html', user=user_data)

@app.route('/manager/farm/weight-assessment')
def manager_farm_weight_assessment():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/weight-settings')
def manager_farm_weight_settings():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/weight-analysis')
def manager_farm_weight_analysis():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
    ...
    return render_template('admin_farm_weight_analysis.html', user=user_data, animal_id=animal_id, litter_id=litter_id)

@app.route('/manager/farm/weight-analytics')
def manager_farm_weight_analytics():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            SELECT l.*, p.tag_id as sow_tag_id, p.breed as sow_breed, f.farm_name as farm_name
            FROM litters l LEFT JOIN pigs p ON l.sow_id = p.id LEFT JOIN farms f ON p.farm_id = f.id
            ORDER BY l.farrowing_date DESC
        """)
        litters = cursor.fetchall()
        cursor.execute("""
            SELECT p.*, f.farm_name, CASE WHEN p.birth_date IS NOT NULL THEN DATEDIFF(CURDATE(), p.birth_date) ELSE NULL END as age_days
            FROM pigs p LEFT JOIN farms f ON p.farm_id = f.id ORDER BY p.created_at DESC
        """)
        pigs = cursor.fetchall()
        cursor.close()
        conn.close()
        return render_template('admin_farm_weight_analytics.html', user=user_data, litters=litters, pigs=pigs)
    except Exception as e:
        print(f"Error loading data: {str(e)}")
        return render_template('admin_farm_weight_analytics.html', user=user_data, litters=[], pigs=[])

@app.route('/manager/farm/vaccinations')
def manager_farm_vaccinations():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/vaccination-settings')
def manager_farm_vaccination_settings():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/cow-feeding-management')
def manager_farm_cow_feeding_management():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
def manager_farm_cow_feeding_tracking():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/pig-feeding-tracking')
def manager_farm_pig_feeding_tracking():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/chicken-feeding-tracking')
def manager_farm_chicken_feeding_tracking():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/cow-feed-settings')
def manager_farm_cow_feed_settings():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/cow-feed-analytics')
def manager_farm_cow_feed_analytics():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/cow-feed-stock-management')
def manager_farm_cow_feed_stock_management():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/chicken-feeding-management')
def manager_farm_chicken_feeding_management():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/chicken-feed-analytics')
def manager_farm_chicken_feed_analytics():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/chicken-feed-stock-management')
def manager_farm_chicken_feed_stock_management():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/health-status')
def manager_farm_health_status():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/pig-analytics')
def manager_farm_pig_analytics():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/litters')
def manager_farm_litters():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
    ...
    return render_template('admin_farm_litters.html', user=user_data)

@app.route('/manager/farm/failed-conceptions')
def manager_farm_failed_conceptions():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
    ...
    return render_template('admin_farm_failed_conceptions.html', user=user_data)

@app.route('/manager/farm/family-tree')
def manager_farm_family_tree():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
    ...
    return render_template('admin_farm_family_tree.html', user=user_data)

@app.route('/manager/farm/register-cows')
def manager_farm_register_cows():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/cow-milk')
def manager_farm_cow_milk():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/milk-production-analytics')
def manager_farm_milk_production_analytics():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/milk-quality-records')
def manager_farm_milk_quality_records():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/cow/<int:cow_id>')
def manager_farm_cow(cow_id):
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
    ...
    return render_template('admin_farm_cow.html', user=user_data, cow_id=cow_id)

@app.route('/manager/farm/animal-details/<int:animal_id>')
def manager_farm_animal_details(animal_id):
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
    ...
    return render_template('admin_farm_animal_details.html', user=user_data, animal_id=animal_id)

@app.route('/manager/farm/vaccination-analytics')
def manager_farm_vaccination_analytics():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
    ...
    return render_template('admin_farm_vaccination_analytics.html', user=user_data)

@app.route('/manager/farm/cow-breeding')
def manager_farm_cow_breeding():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/chicken-flock-management')
def manager_farm_chicken_flock_management():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
...
def manager_farm_chicken_settings():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE TABLE_SCHEMA = DATABASE() 
            AND TABLE_NAME = 'chicken_weight_standards'
        """)
        weight_columns = [row['COLUMN_NAME'] for row in cursor.fetchall()]
        
        if 'start_age' in weight_columns and 'end_age' in weight_columns and 'weight_gain_per_day' in weight_columns:
            cursor.execute("""
                SELECT 
                    id,
                    category,
            ...
            cursor.execute("""
                SELECT 
                    id,
                    category,
                    age_days as start_age,
                    age_days as end_age,
                    expected_weight as weight_gain_per_day,
                    description,
                    created_at
                FROM chicken_weight_standards 
...
@app.route('/manager/farm/chicken-health')
def manager_farm_chicken_health():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
...
@app.route('/manager/farm/chicken-upcoming-medications')
def manager_farm_chicken_upcoming_medications():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
...
@app.route('/manager/farm/chicken-health-analytics')
def manager_farm_chicken_health_analytics():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
...
@app.route('/manager/farm/chicken-production')
def manager_farm_chicken_production():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/manager/farm/chicken-production-management')
def manager_farm_chicken_production_management():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
...
@app.route('/manager/farm/chicken-weight-check')
def manager_farm_chicken_weight_check():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
...
def chicken_registration():
    ...
    try:
        # Get form data and convert text fields to uppercase for consistency
        chicken_id = request.form.get('chicken_id', '').upper().strip()
        batch_name = request.form.get('batch_name', '').upper().strip()
        chicken_type = request.form.get('chicken_type', '').lower().strip()
        breed_name = request.form.get('breed_name', '').upper().strip()
        ...
        # Create chickens table if it doesn't exist (with mixed gender option)
        cursor.execute("""
        ...
        # Alter existing table to add 'mixed' to gender enum if it doesn't exist
        try:
            cursor.execute("""
            ...
            # Column might already have the enum values or table doesn't exist yet
            pass
...
def add_chicken_weight_standard():
    ...
    try:
        ...
        # Check if table exists and has old schema, then alter it
        cursor.execute("""
            SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS 
            WHERE TABLE_SCHEMA = DATABASE() 
            AND TABLE_NAME = 'chicken_weight_standards'
        ...
        # Create or alter chicken_weight_standards table
        if not existing_columns:
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS chicken_weight_standards (
                    ...
                    start_age INT NOT NULL,
                    end_age INT NOT NULL,
                    weight_gain_per_day DECIMAL(8,4) NOT NULL COMMENT 'Weight gain in kg per day',
                    description TEXT,
                    created_by INT NOT NULL,
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                    ...
                    INDEX idx_end_age (end_age),
                    UNIQUE KEY unique_category_age_range (category, start_age, end_age)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
            """)
        else:
            # Alter existing table to add new columns if they don't exist
            if 'start_age' not in existing_columns:
                try:
                    cursor.execute("ALTER TABLE chicken_weight_standards ADD COLUMN start_age INT AFTER category")
                    conn.commit()
                except Exception as e:
                    print(f"Warning: Could not add start_age column: {str(e)}")
            
            if 'end_age' not in existing_columns:
                try:
                    cursor.execute("ALTER TABLE chicken_weight_standards ADD COLUMN end_age INT AFTER start_age")
                    conn.commit()
                except Exception as e:
                    print(f"Warning: Could not add end_age column: {str(e)}")
            
            if 'weight_gain_per_day' not in existing_columns:
                try:
                    cursor.execute("ALTER TABLE chicken_weight_standards ADD COLUMN weight_gain_per_day DECIMAL(8,4) AFTER end_age")
                    conn.commit()
                except Exception as e:
                    print(f"Warning: Could not add weight_gain_per_day column: {str(e)}")
            
            # If old columns exist but new ones don't, we need to handle migration
            if 'age_days' in existing_columns and 'start_age' not in existing_columns:
                # Migrate data: set start_age and end_age to age_days
                try:
                    cursor.execute("""
                        UPDATE chicken_weight_standards 
                        SET start_age = age_days, end_age = age_days 
                        WHERE start_age IS NULL OR end_age IS NULL
                    """)
                    conn.commit()
                except Exception as e:
                    print(f"Warning: Could not migrate age_days data: {str(e)}")
            
            if 'expected_weight' in existing_columns and 'weight_gain_per_day' not in existing_columns:
                # For old data, we can't automatically convert expected_weight to weight_gain_per_day
                # Set a default or leave it NULL (will need manual update)
                try:
                    cursor.execute("""
                        UPDATE chicken_weight_standards 
                        SET weight_gain_per_day = 0.050 
                        WHERE weight_gain_per_day IS NULL
                    """)
                    conn.commit()
                except Exception as e:
                    print(f"Warning: Could not set default weight_gain_per_day: {str(e)}")
            
            # Drop old unique constraint if it exists
            try:
                cursor.execute("ALTER TABLE chicken_weight_standards DROP INDEX unique_category_age")
                conn.commit()
            except Exception as e:
                print(f"Info: unique_category_age index does not exist or already dropped: {str(e)}")
            
            # Add new unique constraint if it doesn't exist
            try:
                cursor.execute("ALTER TABLE chicken_weight_standards ADD UNIQUE KEY unique_category_age_range (category, start_age, end_age)")
                conn.commit()
            except Exception as e:
                print(f"Info: unique_category_age_range constraint may already exist: {str(e)}")
        
        # Refresh column list after potential ALTER TABLE operations
        cursor.execute("""
            SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS 
            WHERE TABLE_SCHEMA = DATABASE() 
            AND TABLE_NAME = 'chicken_weight_standards'
        ...
        # Check if weight standard already exists for this category and age range
        # Handle both old and new schema
        if 'start_age' in final_columns and 'end_age' in final_columns:
            cursor.execute("""
                SELECT id FROM chicken_weight_standards 
                WHERE category = %s AND start_age = %s AND end_age = %s
            """, (category, start_age, end_age))
        else:
            # Fallback to old schema check
            cursor.execute("""
                SELECT id FROM chicken_weight_standards 
        ...
        # Check for overlapping age ranges (only if new columns exist)
        if 'start_age' in final_columns and 'end_age' in final_columns:
            cursor.execute("""
            ...
            """, (category, start_age, start_age, end_age, end_age, start_age, end_age))
        ...
        if 'start_age' in final_columns and 'end_age' in final_columns and 'weight_gain_per_day' in final_columns:
            # Use new schema
            cursor.execute("""
                INSERT INTO chicken_weight_standards (category, start_age, end_age, weight_gain_per_day, description, created_by)
                VALUES (%s, %s, %s, %s, %s, %s)
            """, (category, start_age, end_age, weight_gain_per_day, description, session['employee_id']))
        else:
            # Fallback to old schema (shouldn't happen if migration worked, but handle gracefully)
            return jsonify({
                'success': False, 
                'message': 'Database schema migration failed. Please contact administrator.'
            })
        ...
        return jsonify({
            'success': True, 
            'message': f'Weight standard added successfully for {category} from {start_age} to {end_age} days!'
        })
        
    except ValueError as e:
        print(f"Validation error adding weight standard: {str(e)}")
        return jsonify({
            'success': False, 
...
def update_chicken_weight_standard(weight_standard_id):
    ...
    try:
        ...
        return jsonify({
            'success': True, 
            'message': f'Weight standard updated successfully for {category} from {start_age} to {end_age} days!'
        })
        
    except Exception as e:
        print(f"Error updating weight standard: {str(e)}")
        return jsonify({
            'success': False, 
            'message': f'Error updating weight standard: {str(e)}'
        })
...
def update_feed_setting(setting_id):
    ...
    try:
        ...
        log_activity(session['employee_id'], 'FEED_SETTING_UPDATED', 
                    f'Updated feed setting ID: {setting_id}')
        ...
        print(f"Error updating feed setting: {str(e)}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'message': f'Failed to update feed setting: {str(e)}'}), 500

@app.route('/api/feed/settings/delete/<int:setting_id>', methods=['DELETE'])
def delete_feed_setting(setting_id):
    ...
    try:
        ...
        # Log activity
        log_activity(session['employee_id'], 'FEED_SETTING_DELETED', 
                    f'Deleted feed setting ID: {setting_id}')
        ...
        return jsonify({
            'success': True,
            'message': 'Feed setting deleted successfully'
        })
        
    except Exception as e:
        print(f"Error deleting feed setting: {str(e)}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'message': f'Failed to delete feed setting: {str(e)}'}), 500

@app.route('/api/feed/analytics/requirements', methods=['GET'])
...
@app.route('/api/chicken/next-id', methods=['GET'])
def get_next_chicken_id():
    """Get the next available chicken ID for a given type"""
    if 'employee_id' not in session:
        return jsonify({'error': 'Unauthorized'}), 401
    
    try:
        chicken_type = request.args.get('type', '').lower()
        
        if not chicken_type or chicken_type not in ['broiler', 'kienyeji', 'layer']:
            return jsonify({'error': 'Invalid chicken type'}), 400
        ...
        prefix = prefix_map[chicken_type]
        
        conn = get_db_connection()
        cursor = conn.cursor()
        
        # Get the maximum number for this prefix
        # Extract numeric part from chicken_id (e.g., CB001 -> 1, CK123 -> 123)
        # Use a simpler approach that handles edge cases
        cursor.execute("""
            SELECT chicken_id
            FROM chickens 
            WHERE chicken_id LIKE %s
            ORDER BY chicken_id DESC
            LIMIT 1
        """, (f"{prefix}%",))
        ...
        if result and result.get('chicken_id'):
            # Extract the numeric part from the last ID
            last_id = result['chicken_id']
            if len(last_id) >= 3:
                try:
                    # Extract numeric suffix (everything after the 2-character prefix)
                    numeric_part = last_id[2:]
        ...
        # Format as prefix + 3 digits (e.g., CB001, CK001, CL001)
        next_id = "{}{:03d}".format(prefix, next_num)
        ...
        print(f"Error generating next chicken ID: {str(e)}")
        return jsonify({'error': str(e)}), 500
...
def _milk_analytics_date_filter(date_column='production_date'):
    """Build WHERE clause and params for milk analytics date filter. date_column is e.g. 'production_date' or 'transaction_date'."""
    ...
    where = ""
    params = []
    if filter_mode == 'day' and date_val:
        where = f" AND {date_column} = %s"
        params.append(date_val)
    elif filter_mode == 'range' and date_from and date_to:
        where = f" AND {date_column} BETWEEN %s AND %s"
        params.extend([date_from, date_to])
    elif filter_mode == 'month' and month_val and year_val:
        where = f" AND MONTH({date_column}) = %s AND YEAR({date_column}) = %s"
        params.extend([int(month_val), int(year_val)])
    elif filter_mode == 'year' and year_val:
        where = f" AND YEAR({date_column}) = %s"
...
def get_milk_production_analytics():
    """Get milk production analytics data. Supports filter_mode=day|range|month|year with date, date_from/date_to, month, year."""
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return jsonify({'error': 'Unauthorized'}), 401
    
    date_where, date_params = _milk_analytics_date_filter('production_date')
    default_where = " WHERE MONTH(production_date) = MONTH(CURRENT_DATE()) AND YEAR(production_date) = YEAR(CURRENT_DATE())"
    trend_default = " WHERE production_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)"
    ...
    try:
        ...
        if date_where:
            last_month = {'last_month_production': 0}
        else:
            cursor.execute("""
        ...
        if date_where:
            cursor.execute("""
                SELECT production_date, SUM(milk_quantity) as daily_production
                FROM milk_production 
                WHERE 1=1
                """ + date_where + """
...
def get_milk_usage_analytics():
    """Get milk usage analytics data. Supports date filter: filter_mode, date, date_from/date_to, month, year."""
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return jsonify({'error': 'Unauthorized'}), 401
    
    date_where, date_params = _milk_analytics_date_filter('transaction_date')
    default_where = " AND MONTH(transaction_date) = MONTH(CURRENT_DATE()) AND YEAR(transaction_date) = YEAR(CURRENT_DATE())"
    ...
    try:
        ...
        cursor.execute("""
            SELECT purpose_of_use, SUM(quantity_used) as total_usage
            FROM milk_sales_usage 
            WHERE transaction_type = 'usage'
            """ + (date_where if date_where else default_where) + """
            GROUP BY purpose_of_use
        """, tuple(date_params) if date_params else ())
        usage_data = cursor.fetchall()
        
        # Format usage data - ensure all expected keys exist (ENUM may return as str or bytes)
        usage_stats = {
            'calf_feeding': 0.0,
        ...
        for record in usage_data:
            purpose = record.get('purpose_of_use')
            if purpose is not None:
                key = (purpose.decode('utf-8') if isinstance(purpose, bytes) else str(purpose)).strip()
                if key in usage_stats:
                    usage_stats[key] = float(record['total_usage'] or 0)
...
def get_milk_quality_analytics():
    """Get milk quality analytics data. Supports date filter: filter_mode, date, date_from/date_to, month, year."""
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return jsonify({'error': 'Unauthorized'}), 401
    
    date_where, date_params = _milk_analytics_date_filter('production_date')
    default_where = " AND MONTH(production_date) = MONTH(CURRENT_DATE()) AND YEAR(production_date) = YEAR(CURRENT_DATE())"
    ...
    try:
        ...
        cursor.execute("""
            SELECT milk_quality_assessment, COUNT(*) as count,
                AVG(fat_percentage) as avg_fat, AVG(protein_percentage) as avg_protein
            FROM milk_production 
            WHERE 1=1
            """ + (date_where if date_where else default_where) + """
        ...
        cursor.execute("""
            SELECT AVG(fat_percentage) as avg_fat, AVG(protein_percentage) as avg_protein
            FROM milk_production 
            WHERE 1=1
            """ + (date_where if date_where else default_where) + """
            AND fat_percentage IS NOT NULL AND protein_percentage IS NOT NULL
        """, tuple(date_params) if date_params else ())
...
def get_milk_sales_analytics():
    """Get milk sales analytics data. Supports date filter: filter_mode, date, date_from/date_to, month, year."""
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return jsonify({'error': 'Unauthorized'}), 401
    
    date_where, date_params = _milk_analytics_date_filter('transaction_date')
    default_where = " AND MONTH(transaction_date) = MONTH(CURRENT_DATE()) AND YEAR(transaction_date) = YEAR(CURRENT_DATE())"
    trend_default = " AND transaction_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)"
    ...
    try:
        ...
        cursor.execute("""
            SELECT SUM(quantity_sold) as total_sold, SUM(total_amount) as total_revenue,
                AVG(price_per_liter) as avg_price, COUNT(*) as sales_count
            FROM milk_sales_usage 
            WHERE transaction_type = 'sale'
            """ + (date_where if date_where else default_where), tuple(date_params) if date_params else ())
        sales_data = cursor.fetchone()
        
        cursor.execute("""
            SELECT buyer, SUM(quantity_sold) as quantity, SUM(total_amount) as revenue
            FROM milk_sales_usage 
            WHERE transaction_type = 'sale'
            """ + (date_where if date_where else default_where) + """
            GROUP BY buyer
        ...
        if date_where:
            cursor.execute("""
                SELECT transaction_date, SUM(quantity_sold) as daily_quantity, SUM(total_amount) as daily_revenue
                FROM milk_sales_usage 
                WHERE transaction_type = 'sale'
                """ + date_where + """
...
def get_animal_production_analytics():
    """Get individual animal production analytics. Supports date filter: filter_mode, date, date_from/date_to, month, year."""
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return jsonify({'error': 'Unauthorized'}), 401
    
    date_where, date_params = _milk_analytics_date_filter('mp.production_date')
    join_date_filter = " AND MONTH(mp.production_date) = MONTH(CURRENT_DATE()) AND YEAR(mp.production_date) = YEAR(CURRENT_DATE())" if not date_where else date_where.replace('mp.', '')
    join_date_filter = " AND " + join_date_filter.lstrip(" AND ") if join_date_filter else ""
    ...
    try:
        ...
        if date_where:
            cursor.execute("""
                ...
                FROM cows c
                LEFT JOIN milk_production mp ON c.id = mp.cow_id AND 1=1
                """ + date_where + """
                WHERE c.status = 'active'
...
def get_cow_detailed_info(cow_id):
    """Get detailed information for a specific cow. Supports date filters: filter_mode=day|range|month|year, date=YYYY-MM-DD, date_from/date_to, month=1-12, year=YYYY."""
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return jsonify({'error': 'Unauthorized'}), 401
    
    filter_mode = request.args.get('filter_mode')  # day, range, month, year
    date_val = request.args.get('date')
    date_from = request.args.get('date_from')
    date_to = request.args.get('date_to')
    month_val = request.args.get('month')
    ...
    date_params = []
    if filter_mode == 'day' and date_val:
        date_where = " AND mp.production_date = %s"
        date_params.append(date_val)
    elif filter_mode == 'range' and date_from and date_to:
        date_where = " AND mp.production_date BETWEEN %s AND %s"
        date_params.extend([date_from, date_to])
    elif filter_mode == 'month' and month_val and year_val:
        date_where = " AND MONTH(mp.production_date) = %s AND YEAR(mp.production_date) = %s"
        date_params.extend([int(month_val), int(year_val)])
    elif filter_mode == 'year' and year_val:
        date_where = " AND YEAR(mp.production_date) = %s"
        date_params.append(int(year_val))
    
    stats_date_where = date_where.replace("mp.", "") if date_where else ""
    ...
    try:
        ...
        # Get milk production history for this cow (with optional date filter)
        cursor.execute("""
            SELECT 
                mp.id,
                mp.production_date,
            ...
            LIMIT 500
        """, (cow_id,) + tuple(date_params))
        production_history = cursor.fetchall()
        
        # Get production statistics for this cow (with same date filter)
        cursor.execute("""
            SELECT 
            ...
            FROM milk_production 
            WHERE cow_id = %s
            """ + stats_date_where, (cow_id,) + tuple(date_params))
        stats = cursor.fetchone()
        
        # Production trend: for day show that day; for range/month/year show daily totals in range
        if filter_mode == 'day' and date_val:
            cursor.execute("""
                SELECT 
                    production_date,
                ...
                FROM milk_production 
                WHERE cow_id = %s AND production_date = %s
                GROUP BY production_date
                ORDER BY production_date
            """, (cow_id, date_val))
        elif filter_mode == 'range' and date_from and date_to:
            cursor.execute("""
                SELECT 
                    ...
                    SUM(milk_quantity) as daily_total
                FROM milk_production 
                WHERE cow_id = %s AND production_date BETWEEN %s AND %s
                GROUP BY production_date
                ORDER BY production_date
            """, (cow_id, date_from, date_to))
        elif filter_mode == 'month' and month_val and year_val:
            cursor.execute("""
                SELECT 
                    production_date,
                    SUM(milk_quantity) as daily_total
                FROM milk_production 
                WHERE cow_id = %s AND MONTH(production_date) = %s AND YEAR(production_date) = %s
                GROUP BY production_date
                ORDER BY production_date
            """, (cow_id, int(month_val), int(year_val)))
        elif filter_mode == 'year' and year_val:
            cursor.execute("""
                SELECT 
                    production_date,
                    SUM(milk_quantity) as daily_total
                FROM milk_production 
                WHERE cow_id = %s AND YEAR(production_date) = %s
                GROUP BY production_date
                ORDER BY production_date
            """, (cow_id, int(year_val)))
        else:
...
# ----- Cow Vaccination APIs (same logic as pig vaccination) -----
@app.route('/api/cow-vaccination/animals', methods=['GET'])
def get_cow_vaccination_animals():
    if 'employee_id' not in session:
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("""
            SELECT id, ear_tag, name, breed, gender, birth_date, age_days, source, status, created_at
            FROM cows WHERE status = 'active' ORDER BY created_at DESC
        """)
        cows = cursor.fetchall()
        cursor.close()
        conn.close()
        ...
        print(f"Error getting cow vaccination animals: {str(e)}")
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/cow-vaccination/schedule', methods=['GET'])
def get_cow_vaccination_schedule():
    if 'employee_id' not in session:
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("""
            SELECT id, day_number, day_description, reason, medicine_activity, dosage_amount,
                   interval_duration, additional_notes, medicine_image, animal_image, created_at, updated_at
            FROM cow_vaccination_schedule ORDER BY day_number ASC
        """)
        schedule_entries = cursor.fetchall()
        cursor.close()
        conn.close()
        return jsonify({'success': True, 'schedule': schedule_entries})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/cow-vaccination/schedule/<int:schedule_id>', methods=['GET'])
def get_cow_vaccination_schedule_entry(schedule_id):
    if 'employee_id' not in session:
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        conn = get_db_connection()
        ...
        cursor.execute("""
            SELECT id, day_number, day_description, reason, medicine_activity, dosage_amount,
                   interval_duration, additional_notes, medicine_image, animal_image, created_at, updated_at
            FROM cow_vaccination_schedule WHERE id = %s
        """, (schedule_id,))
        entry = cursor.fetchone()
        cursor.close()
        conn.close()
...
@app.route('/api/cow-vaccination/schedule', methods=['POST'])
def add_cow_vaccination_schedule():
    if 'employee_id' not in session:
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        data = request.get_json() or request.form.to_dict()
        for f in ['day_number', 'reason', 'medicine_activity']:
            if not data.get(f):
                return jsonify({'success': False, 'error': f'Missing {f}'}), 400
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id FROM cow_vaccination_schedule WHERE day_number = %s", (data['day_number'],))
        if cursor.fetchone():
            cursor.close()
            conn.close()
            return jsonify({'success': False, 'error': 'Day number exists'}), 400
        cursor.execute("""
            INSERT INTO cow_vaccination_schedule (day_number, day_description, reason, medicine_activity, dosage_amount,
                interval_duration, additional_notes, medicine_image, animal_image, created_by)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        """, (
            data['day_number'], data.get('day_description', ''),
            data['reason'], data['medicine_activity'], data.get('dosage_amount', ''),
            data.get('interval_duration', ''), data.get('additional_notes', ''),
            data.get('medicine_image', ''), data.get('animal_image', ''),
            session['employee_id']
        ))
        conn.commit()
        ...
        cursor.close()
        conn.close()
        return jsonify({'success': True, 'message': 'Added', 'id': rid})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/cow-vaccination/schedule/<int:schedule_id>', methods=['PUT'])
def update_cow_vaccination_schedule(schedule_id):
    if 'employee_id' not in session:
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        data = request.get_json() or request.form.to_dict()
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id FROM cow_vaccination_schedule WHERE id = %s", (schedule_id,))
        if not cursor.fetchone():
            cursor.close()
            conn.close()
            return jsonify({'success': False, 'error': 'Not found'}), 404
        cursor.execute("""
            UPDATE cow_vaccination_schedule SET day_number=%s, day_description=%s, reason=%s, medicine_activity=%s,
                dosage_amount=%s, interval_duration=%s, additional_notes=%s, medicine_image=%s, animal_image=%s, updated_at=CURRENT_TIMESTAMP
            WHERE id=%s
        """, (
            data.get('day_number'), data.get('day_description', ''), data.get('reason'), data.get('medicine_activity'),
            data.get('dosage_amount', ''), data.get('interval_duration', ''), data.get('additional_notes', ''),
            data.get('medicine_image', ''), data.get('animal_image', ''), schedule_id
        ))
        conn.commit()
        cursor.close()
        conn.close()
...
@app.route('/api/cow-vaccination/schedule/<int:schedule_id>', methods=['DELETE'])
def delete_cow_vaccination_schedule(schedule_id):
    if 'employee_id' not in session:
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("DELETE FROM cow_vaccination_schedule WHERE id = %s", (schedule_id,))
        conn.commit()
        cursor.close()
        conn.close()
...
@app.route('/api/cow-vaccination/complete', methods=['POST'])
def mark_cow_vaccination_completed():
    if 'employee_id' not in session:
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        data = request.get_json()
        for f in ['cow_id', 'schedule_id', 'completion_notes']:
            if f not in data:
                return jsonify({'success': False, 'error': f'Missing {f}'}), 400
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("""
            SELECT id FROM cow_vaccination_records WHERE cow_id = %s AND schedule_id = %s
        """, (int(data['cow_id']), int(data['schedule_id'])))
        if cursor.fetchone():
            cursor.close()
            conn.close()
            return jsonify({'success': False, 'error': 'Already completed'}), 400
        cursor.execute("""
            INSERT INTO cow_vaccination_records (cow_id, schedule_id, completed_date, completion_notes, completed_by)
            VALUES (%s, %s, %s, %s, %s)
        """, (int(data['cow_id']), int(data['schedule_id']), datetime.now().date(), data['completion_notes'], session['employee_id']))
        conn.commit()
        cursor.close()
        conn.close()
        return jsonify({'success': True, 'message': 'Vaccination marked completed'})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/cow-vaccination/records/<int:cow_id>', methods=['GET'])
def get_cow_vaccination_records(cow_id):
    if 'employee_id' not in session:
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("""
            SELECT vr.*, vs.day_number, vs.day_description, vs.medicine_activity, vs.reason, e.full_name as completed_by_name
            FROM cow_vaccination_records vr
            JOIN cow_vaccination_schedule vs ON vr.schedule_id = vs.id
            JOIN employees e ON vr.completed_by = e.id
            WHERE vr.cow_id = %s ORDER BY vr.completed_date DESC
        """, (cow_id,))
        records = cursor.fetchall()
        cursor.close()
        conn.close()
...
# ----- Cow Weight APIs -----
@app.route('/api/cow-weight/settings', methods=['GET'])
def get_cow_weight_settings():
    if 'employee_id' not in session:
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT setting_name, setting_value, setting_type, description FROM cow_weight_settings ORDER BY setting_name")
        rows = cursor.fetchall()
        settings_dict = {r['setting_name']: {'value': r['setting_value'], 'type': r['setting_type'], 'description': r['description']} for r in rows}
        cursor.close()
        conn.close()
        return jsonify({'success': True, 'settings': settings_dict})
    except Exception as e:
...
@app.route('/api/cow-weight/settings', methods=['POST'])
def save_cow_weight_settings():
    if 'employee_id' not in session:
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        data = request.get_json()
        ...
        for name, val in (data or {}).items():
            cursor.execute("""
                INSERT INTO cow_weight_settings (setting_name, setting_value, setting_type, description, created_by)
                VALUES (%s, %s, 'text', '', %s) ON DUPLICATE KEY UPDATE setting_value = VALUES(setting_value), updated_at = CURRENT_TIMESTAMP
            """, (name, str(val), session['employee_id']))
        conn.commit()
        cursor.close()
        conn.close()
...
@app.route('/api/cow-weight/categories', methods=['GET'])
def get_cow_weight_categories():
    if 'employee_id' not in session:
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id, category_name, min_weight, max_weight, daily_gain, start_age, end_age FROM cow_weight_categories ORDER BY start_age ASC")
        categories = cursor.fetchall()
        cursor.close()
        conn.close()
...
@app.route('/api/cow-weight/categories', methods=['POST'])
def add_cow_weight_category():
    if 'employee_id' not in session:
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        data = request.get_json()
        for f in ['start_age', 'end_age', 'category_name', 'min_weight', 'max_weight', 'daily_gain']:
            if f not in data:
                return jsonify({'success': False, 'error': f'Missing {f}'}), 400
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO cow_weight_categories (start_age, end_age, category_name, min_weight, max_weight, daily_gain, created_by)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """, (data['start_age'], data['end_age'], data['category_name'], data['min_weight'], data['max_weight'], data['daily_gain'], session['employee_id']))
        conn.commit()
        rid = cursor.lastrowid
        cursor.close()
        conn.close()
...
@app.route('/api/cow-weight/categories/<int:category_id>', methods=['PUT'])
def update_cow_weight_category(category_id):
    if 'employee_id' not in session:
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        data = request.get_json()
        ...
        cursor = conn.cursor()
        cursor.execute("""
            UPDATE cow_weight_categories SET start_age=%s, end_age=%s, category_name=%s, min_weight=%s, max_weight=%s, daily_gain=%s, updated_at=CURRENT_TIMESTAMP
            WHERE id=%s
        """, (data.get('start_age'), data.get('end_age'), data.get('category_name'), data.get('min_weight'), data.get('max_weight'), data.get('daily_gain'), category_id))
        conn.commit()
        cursor.close()
        conn.close()
        return jsonify({'success': True})
...
@app.route('/api/cow-weight/categories/<int:category_id>', methods=['DELETE'])
def delete_cow_weight_category(category_id):
    if 'employee_id' not in session:
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("DELETE FROM cow_weight_categories WHERE id = %s", (category_id,))
        conn.commit()
        cursor.close()
        conn.close()
        return jsonify({'success': True})
...
@app.route('/api/cow-weight/records', methods=['GET'])
def get_cow_weight_records():
    ...
    try:
        ...
        if cow_id:
            cursor.execute("""
                SELECT w.*, c.ear_tag, c.name as cow_name FROM cow_weight_records w
                JOIN cows c ON w.cow_id = c.id WHERE w.cow_id = %s ORDER BY w.weighing_date DESC
            """, (cow_id,))
        else:
            cursor.execute("""
                SELECT w.*, c.ear_tag, c.name as cow_name FROM cow_weight_records w
                JOIN cows c ON w.cow_id = c.id ORDER BY w.weighing_date DESC LIMIT 200
            """)
        records = cursor.fetchall()
        cursor.close()
        conn.close()
        for r in records:
            if r.get('weighing_date'):
                r['weighing_date'] = str(r['weighing_date'])
        return jsonify({'success': True, 'records': records})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/cow-weight/records', methods=['POST'])
def add_cow_weight_record():
    if 'employee_id' not in session:
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        data = request.get_json()
        for f in ['cow_id', 'weight', 'weighing_date']:
            if f not in data:
                return jsonify({'success': False, 'error': f'Missing {f}'}), 400
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO cow_weight_records (cow_id, weight, expected_weight, weight_type, weighing_date, notes, created_by)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """, (int(data['cow_id']), float(data['weight']), data.get('expected_weight'), data.get('weight_type', 'actual'), data['weighing_date'], data.get('notes', ''), session['employee_id']))
        conn.commit()
        cursor.close()
        conn.close()
        return jsonify({'success': True, 'message': 'Weight record added'})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def milk_quality_records_page():
    """Milk Quality Records page - production, quality, sales and usage analytics"""
    if 'employee_id' not in session:
        return redirect(url_for('employee_login'))
...
@app.route('/admin/farm/cow-health')
def admin_farm_cow_health():
    """Cow Health page - vaccine & weight links only on this page"""
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
...
@app.route('/admin/farm/cow-vaccination-tracking')
def admin_farm_cow_vaccination_tracking():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/admin/farm/cow-vaccination-settings')
def admin_farm_cow_vaccination_settings():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/admin/farm/cow-weight-assessment')
def admin_farm_cow_weight_assessment():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
@app.route('/admin/farm/cow-weight-settings')
def admin_farm_cow_weight_settings():
    if 'employee_id' not in session or session.get('employee_role') not in ['administrator', 'manager']:
        return redirect(url_for('employee_login'))
    user_data = {
        'id': session['employee_id'], 'name': session['employee_name'],
...
# Run DB/tables/columns check and migrations when app is loaded (local and hosted).
# When you pull from GitHub on the host, this runs and applies any new migrations.
try:
