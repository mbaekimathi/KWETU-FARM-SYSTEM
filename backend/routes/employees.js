...
router.put('/approve/:employeeId', authenticateToken, requireRole(['admin']), [
...
// Admin: Update Employee Details (Enhanced version with full editing)
router.put('/update/:employeeId', authenticateToken, requireRole(['admin']), [
...
router.put('/status/:employeeId', authenticateToken, requireRole(['admin']), [
...
// Admin: Bulk Actions
router.post('/bulk-action', authenticateToken, requireRole(['admin']), [
 body('action').isIn(['activate', 'suspend', 'delete', 'update_department']).withMessage('Invalid bulk action'),
 body('employeeIds').isArray({ min: 1 }).withMessage('Employee IDs array is required'),
 body('employeeIds.*').isInt().withMessage('Each employee ID must be a valid integer')
], async (req, res) => {
 try {
   const errors = validationResult(req);
   if (!errors.isEmpty()) {
   ...
   const { action, employeeIds, department } = req.body;
   
   // Prevent self-actions
   if (employeeIds.includes(req.user.employeeId)) {
     return res.status(400).json({ 
       success: false,
   ...
   switch (action) {
     case 'activate':
       [result] = await promisePool.execute(
         'UPDATE employees SET status = "active", updated_at = NOW() WHERE id IN (?) AND role != "waiting_approval"',
         [employeeIds]
       );
       message = `${result.affectedRows} employees activated successfully`;
       break;

     case 'suspend':
       [result] = await promisePool.execute(
         'UPDATE employees SET status = "suspended", updated_at = NOW() WHERE id IN (?) AND role != "waiting_approval"',
         [employeeIds]
       );
       message = `${result.affectedRows} employees suspended successfully`;
       break;
     ...
     case 'update_department':
       if (!department) {
         return res.status(400).json({ 
           success: false,
           message: 'Department is required for department update action'
         });
       }
       [result] = await promisePool.execute(
         'UPDATE employees SET department = ?, updated_at = NOW() WHERE id IN (?) AND role != "waiting_approval"',
         [department, employeeIds]
       );
       message = `${result.affectedRows} employees moved to ${department} department`;
       break;

     case 'delete':
       // Check for admins in the list
       const [adminCheck] = await promisePool.execute(
         'SELECT COUNT(*) as admin_count FROM employees WHERE id IN (?) AND role = "admin"',
         [employeeIds]
       );
       ...
       if (adminCheck[0].admin_count > 0) {
         return res.status(400).json({ 
           success: false,
           message: 'Cannot bulk delete admin accounts. Please change their roles first.'
         });
       }

       [result] = await promisePool.execute(
         'DELETE FROM employees WHERE id IN (?) AND role != "waiting_approval"',
         [employeeIds]
       );
...
function calculateProfileCompleteness(employee) {
 const fields = ['full_name', 'email', 'phone_number', 'department', 'position', 'salary'];
 const completedFields = fields.filter(field => employee[field] && employee[field] !== null).length;
...
function formatWaitingTime(days) {
...
function calculateGrowthRate(newRegistrations, totalEmployees) {
 if (totalEmployees === 0) return 0;
 return ((newRegistrations / totalEmployees) * 100).toFixed(1);
}
